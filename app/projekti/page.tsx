...
// Funkcija za dobijanje PROJEKATA
async function getProjects() {
  const { data: projects, error } = await supabase
    .from("posts")
    .select("*")
    .eq("type", "project") // Filtriramo samo projekte
...
// ---------------------------------------------------------
// NOVA FUNKCIJA: Čisti HTML tagove iz teksta za ljepši prikaz na kartici
// ---------------------------------------------------------
function stripHtml(html: string) {
  if (!html) return "";
  // 1. Zamijeni HTML tagove sa praznim stringom ili razmakom
        ...
        <RevealSection>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {projekti.map((project: any) => {
                  // Ovdje pripremamo čist tekst za prikaz
                  const cleanDescription = stripHtml(project.content || "");
                  ...
                  return (
                    <div key={project.id} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 border border-gray-100 group flex flex-col">
                        ...
                        <div className="p-8 flex flex-col flex-grow">
                            ...
                            {/* OVDJE JE IZMJENA: Koristimo očišćeni tekst */}
                            <p className="text-gray-600 line-clamp-3 mb-6 flex-grow">
                                {cleanDescription.length > 120 
                                  ? cleanDescription.substring(0, 120) + "..." 
                                  : cleanDescription || "Nema dodatnog opisa."}
                            </p>
                            
                            <Link href={`/projekti/${project.id}`} className="inline-flex items-center font-bold text-green-700 hover:text-green-900 transition mt-auto">
